...
import {
    ...
    ArchiveIcon,
    XIcon
} from './icons'; // Assuming icons.jsx is in ./icons/

function BatchCard({ batch, onUpdateBatch, onMoveBatch, onDeleteBatch, onOpenHarvestModal }) {
    const [showHarvests, setShowHarvests] = useState(false);
    const [colonisationDateInput, setColonisationDateInput] = useState(batch?.colonisationCompleteDate || ''); // Use optional chaining
    const [showColonisationInput, setShowColonisationInput] = useState(false);

    // Ensure batch exists before accessing properties
    ...
    const handleRemoveHarvest = (index) => {
        if (!Array.isArray(safeBatch.harvests)) return; // Check if harvests is an array
    ...
    // Calculate total harvested weight safely
    const totalHarvested = Array.isArray(safeBatch.harvests)
        ? safeBatch.harvests.reduce((sum, h) => sum + (h?.weight || 0), 0).toFixed(2)
        : "0.00";

    // Styles
    const cardBaseStyle = "bg-white border border-gray-200 rounded-lg shadow-sm p-4 text-sm text-gray-700 flex flex-col h-full"; // Added flex for layout
    ...
    return (
        <div className={cardBaseStyle}>
            ...
            {/* Action Buttons Area */}
            <div className="mt-auto pt-3 border-t border-gray-200"> {/* Push buttons to bottom */}
                {safeBatch.stage === 'incubation' && !safeBatch.colonisationCompleteDate && (
                    <div className="mb-3"> {/* Add margin below date input */}
                ...
                <div className="flex flex-wrap gap-2">
                    {safeBatch.stage === 'incubation' && ( <> <button onClick={() => onMoveBatch(safeBatch.id, 'grow')} className={primaryButtonStyle} disabled={!safeBatch.colonisationCompleteDate} title={!safeBatch.colonisationCompleteDate ? "Set colonisation date first" : "Move to Grow Room"}> Move to Grow <ArrowRightIcon /> </button> <button onClick={() => onDeleteBatch(safeBatch.id)} className={dangerButtonStyle}><TrashIcon /> Delete</button> </> )}
                    {safeBatch.stage === 'grow' && ( <> <button onClick={() => onOpenHarvestModal(safeBatch.id)} className={primaryButtonStyle}><PlusIcon /> Log Harvest</button> <button onClick={() => onMoveBatch(safeBatch.id, 'incubation')} className={secondaryButtonStyle}><ArrowLeftIcon /> Back to Incubation</button> <button onClick={() => onMoveBatch(safeBatch.id, 'retired')} className={secondaryButtonStyle}><ArchiveIcon /> Retire Batch</button> </> )}
                    {safeBatch.stage === 'retired' && ( <> <button onClick={() => onMoveBatch(safeBatch.id, 'grow')} className={primaryButtonStyle}><ArrowLeftIcon /> Move Back to Grow</button> </> )}
                </div>
            </div>
        </div>
    );
